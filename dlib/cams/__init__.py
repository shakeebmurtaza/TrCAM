...
def build_std_cam_extractor(classifier, args):
    ...
    if args.seed_map_method == constants.METHOD_CLIPCAM:
        std_cam_extactor = Get_CLIP_ATTN(model=classifier, args=args)
        # load_dino_pretrained_weights(std_cam_extactor.model, '', 'teacher', 'vit_small', 16)
        std_cam_extactor.cuda(args.c_cudaid)
        
        return std_cam_extactor
    elif method == constants.METHOD_CAM and args.model['arch'] == constants.VIT_CLASSIFIER:
        return ViT_Get_Attn(model=classifier, args=args)
