...
from dlib.datasets import wsol_utils
            ...
            if self.dataset_mode == constants.DS_SHOTS:
                ...
                for zz in temporal_frames:
                    ...
                    if std_cam is None:
                        ...
                        std_cam = torch.cat((std_cam, c_cam), dim=0)
                        # std_cam = torch.maximum(std_cam, c_cam)  # 1,
                        # h', w'
                std_cam = std_cam * torch.tensor(wsol_utils.compute_weights(n=std_cam.shape[0], alpha=1, mode='log', sl_tc_knn_mode=self.args.sl_tc_knn_mode)).unsqueeze(1).unsqueeze(2)#self.adjasmet_frames_weights.unsqueeze(1).unsqueeze(2)
                std_cam = std_cam.mean(dim=0).unsqueeze(0)  # 1, h', w'
                # print(f'considering n: {std_cam.shape[0]} frames')
            else:
                std_cam = self.load_cam(image_id)
