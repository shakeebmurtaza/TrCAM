...
class Trainer(Basic):
    ...
    def get_std_cams_minibatch(self, images, targets, img_ids = None) -> torch.Tensor:
        # used only for task f_cl
        assert self.args.task == constants.F_CL or self.args.task == constants.TCAM
        ...
        for idx, (image, target) in enumerate(zip(images, targets)):
            ...
            if self.args.seed_map_method == constants.METHOD_CLIPCAM:
                cam = self.std_cam_extractor(x=image.unsqueeze(0),
                class_idx=target.item(), normalized=True, img_id = img_ids[idx])
            else:
            ...
            if self.is_seed_required(_epoch=self.epoch):
                if std_cams is None:
                    cams_inter = self.get_std_cams_minibatch(images=images,
                                                             targets=targets, img_ids=images_id)
                else:
                    cams_inter = std_cams
                ...
                if _cnd:  # todo: stop loading classifier's cams in dataset.
                    ...
                    if std_cams is None:
                        cams_inter = self.get_std_cams_minibatch(
                            images=images, targets=targets, img_ids=images_id)
                    else:
                        cams_inter = std_cams
    ...
    # ========= save cams to train =========
    def save_cams_before_training(self, split,):
        ...
        loader, _ = get_data_loader(
                        ...
                        isdistributed=False)
        loader = loader[split]
        if self.args.dataset in [constants.YTOV1, constants.YTOV22]:
            loader.dataset._switch_to_frames_mode()
        ...
        for idx, (images, targets, image_ids, raw_imgs, _, _, _, _) in tqdm(
            ...
            cams_inter = self.get_std_cams_minibatch(images=images,
                                                             targets=targets, img_ids=image_ids)
    ...
    def cl_forward(self, images: torch.Tensor, blured_imgs: torch.Tensor=None):
        ...
        elif self.args.task == constants.LSA:
            cl_logits, cams = output
        else:
            raise NotImplementedError
    ...
    def evaluate(self, epoch, split, checkpoint_type=None, fcam_argmax=False):
        ...
        if split == constants.TESTSET and self.args.is_master and self.args.task == constants.TCAM:
    ...
    def load_checkpoint(self, checkpoint_type, net=None):
        ...
        if self.args.task == constants.STD_CL:
            ...
            if box_head_w is not None:
                box_head_w = move_state_dict_to_device(model_w, self.device)
                net.model.load_state_dict(model_w, strict=True)
