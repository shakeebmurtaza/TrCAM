...
class CAMComputer(object):
    ...
    def get_cam_one_sample(self,
                           ...
                           img_id = None) -> Tuple[torch.Tensor, torch.Tensor,
                                      Union[dict, None]]:
        ...
        with autocast(enabled=self.args.amp_eval):
            if self.args.method == constants.METHOD_CLIPCAM:
                output = torch.zeros(1, self.args.num_classes)#return self.model(image, img_id)
            else:
        ...
        if task == constants.STD_CL:
            ...
            cl_logits = output
            if self.args.method == constants.METHOD_CLIPCAM:
                cam = self.std_cam_extractor(x=image, normalized = True, img_id=img_id, class_idx=target).squeeze()
            elif self.args.model['arch'] == constants.VIT_CLASSIFIER:
                cam = self.std_cam_extractor(x=image, normalized = True).squeeze()
            else:
    ...
    def minibatch_accum(self, images, targets, image_ids, image_size,
        ...
        for image, target, image_id in zip(images, targets, image_ids):
            # if image_id not in constants_img_lst.img_lst:
                # continue
            ...
            with torch.no_grad():
                ...
                # if self.split in ['test']: #('val', 'test'):
                #     cam_path = os.path.join(self.out_folder, 'scoremaps', image_id.replace('/', '_'))
                #     if not os.path.exists(os.path.join(self.out_folder, 'scoremaps')):
                #         os.makedirs(os.path.join(self.out_folder, 'scoremaps'))
                #     np.save(os.path.join(cam_path), cam)
        ...
        for _image_id in tqdm(ids_to_draw, ncols=constants.NCOLS, total=len(
            ...
            with torch.set_grad_enabled(self.req_grad):
                ...
                low_cam, cl_logits, cbox = self.get_cam_one_sample(
                    image=image.unsqueeze(0), target=target, img_id=_image_id)
            ...
            if isinstance(self.evaluator, BoxEvaluator) or self.evaluator is \
                ...
                for tau in taux:
                    ...
                    if separate:
                        outf = join(th_fd, '{}.png'.format(reformat_id(
                            image_id)))
                        self.viz.plot_single(datum=l_datum[-1], outf=outf, plot_all_instances=plot_all_instances)
