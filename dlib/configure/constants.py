...
#Hyperparameters searching optimizers
OPTIMIZER_COMET = 'comet'
OPTIMIZER_OPTUNA = 'optuna'
OPTIMIZERS_FOR_HYPERPARAMS_SEARCH = [OPTIMIZER_COMET, OPTIMIZER_OPTUNA]
# possible tasks
...
LSA = 'LSA' # Localization with Slot Attention
C_BOX = 'C_BOX'  # c-box task: weakly supervised object localization.
SEG = "SEGMENTATION"  # standard supervised segmentation. outputs:
# segmentation masks.
# outputs: logits, cams.
TCAM_DINO = 'TCAM_DINO'  # temporal CAM for wsol for videos using transformer
# outputs: logits, cams.
VIT_CLASSIFIER = 'vit_classifier'
VIT_LOCALIZER = 'vit_localizer'
SLOTATTENTION_VITBACKBONE = 'SlotAttention_vitbackbone'
TASKS = [STD_CL, F_CL, C_BOX, TCAM, SEG, TCAM_DINO, LSA]
...
METHOD_VITATTN = 'VIT_ATTN'
METHOD_SlotAttention = 'SlotAttention'
...
VITS_WITH_LOC_HEAD = 'vits_with_loc_head'  # Newly added
...
ARCHS = [STDCLASSIFIER, UNETFCAM, UNETTCAM, DENSEBOXNET, VITS_WITH_LOC_HEAD, VIT_CLASSIFIER, VIT_LOCALIZER, SLOTATTENTION_VITBACKBONE]
...
if "CC_CLUSTER" in os.environ.keys():
    DATA_CAMS = os.path.join(os.environ['SLURM_TMPDIR'], DATA_CAMS)
...
LOCALIZATION_MTR_PERCLASS = 'localization_per_class'
