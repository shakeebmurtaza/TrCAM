...
from dlib.slotattentionmodel_vitbackbone.slotattention_model import SlotAttentionModel_ViTBackbone
...
def get_loss_lsa(args):
    ...
    # image classification loss
    assert args.task == constants.LSA
...
def get_loss(args):
    ...
    elif args.task == constants.LSA:
        masterloss = get_loss_lsa(args)
    else:
...
def get_pretrainde_classifier(args, pretrained_ch_pt: str, _model_with_backbone=None):
    ...
    if _model_with_backbone is None:
        ...
        model = _model_with_backbone.classification_head
        DLLogger.log("PRETRAINED CLASSIFIER `{}` was created. "
                    "Nbr.params: {}".format(model, count_nb_params(model)))
        log = f"Arch: {p.arch}\n" \
            f"encoder_name: {p.encoder_name}\n" \
            f"encoder_weights: {encoder_weights}\n" \
            f"classes: {classes}\n" \
            f"freeze_cl: classifier always freezed with ViT\n"
        DLLogger.log(log)
        pretrained_ch_pt = constants.BEST_CL
        model.set_classifier_frozen_flag(True)
...
def get_model(args, eval=False):
    ...
    if args.task == constants.LSA and p.arch == constants.SLOTATTENTION_VITBACKBONE:
        model =  SlotAttentionModel_ViTBackbone(vit_backbone=p.encoder_name, args=args, num_labels=classes)
        load_dino_pretrained_weights(model.backbone, '', 'teacher', p.encoder_name, p.ssl_patch_size)
        return model
    elif args.task == constants.STD_CL and args.model['arch'] == constants.VIT_CLASSIFIER:
        model =  ViT_Classifer(args, num_labels=classes)
        load_dino_pretrained_weights(model.encoder, '', 'teacher', p.encoder_name, p.ssl_patch_size)
        return model
    elif args.task == constants.TCAM and args.model['arch'] == constants.VIT_LOCALIZER:
...
def _get_model_params_for_opt(args, model):
    ...
    if args.task in [constants.F_CL, constants.C_BOX, constants.TCAM]:
        ...
        return [
            {'params': model.classification_head.parameters(), 'lr': hparams.lr}
        ]
    elif  args.task == constants.LSA and args.model['arch'] == constants.SLOTATTENTION_VITBACKBONE:
        return [
            {'params': model.model.parameters(), 'lr': hparams.lr}
        ]
