...
class Get_CLIP_ATTN(nn.Module):
    ...
    def __init__(self, model, args, save_cams=True):
        ...
        #=============get class names================
        assert args.dataset in [constants.YTOV1, constants.YTOV22, constants.OpenImages, constants.ILSVRC], f"{args.dataset} dataset not supported to obtain seeds from CLIP"
        self.class_name = []
        if args.dataset == constants.OpenImages:
            ...
            self.classes_ids = {}
            self.classes_label_for_each_img = {}
            ...
            with open(join(args.metadata_root, 'train', 'class_labels.txt'), 'r') as fin:
                for line in fin.readlines():
                    ...
                    self.classes_label_for_each_img[line.split(',')[0]] = {'class_name': class_name, 'class_label': class_id}
                    
            for split_for_labels in [constants.VALIDSET, constants.VALSET]:
                with open(join(args.metadata_root, split_for_labels, 'image_ids.txt'), 'r') as fin:
                    class_id = int(line.split(',')[1].replace('\n', ''))
                    class_name = imagenet_labels[class_id]
                    self.classes_label_for_each_img[line.split(',')[0]] = {'class_name': class_name, 'class_label': class_id}
            for class_id_indx in range(len(self.classes_ids.keys())):
                self.class_name.append(self.classes_ids[class_id_indx])
        else:
            root = get_root_wsol_dataset()
            root_ds = join(root, args.dataset)
